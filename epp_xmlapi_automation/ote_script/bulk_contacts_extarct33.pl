...
my $ms="/root/eppclient_prod_client/eppclient/replay_tool/ote_script";
my $PERL="/bin/perl";
my $FILEPATH="/root/eppclient_prod_client/eppclient/replay_tool/ote_script";
...
my $cmd1 = "/bin/perl /root/eppclient_prod_client/eppclient/replay_tool/ote_script/bulk_contacts_extarct2.pl";`$cmd1`;
...
while ($_ = <$A> . <$B> . <$C> . <$D> . <$E>) {
       ...
       for (my $i=0;$i<=$#E5;$i++){
         ...
         $voicer=$config2->{response}->{resData}->{"contact:infData"}->{"contact:voice"};#print "\n$voicer\n";
         $regemail2=$config3->{response}->{resData}->{"contact:infData"}->{"contact:email"};#print "\n$regemail2\n";
         $voicea=$config3->{response}->{resData}->{"contact:infData"}->{"contact:voice"};#print "\n$voicea\n";
         $regemail3=$config4->{response}->{resData}->{"contact:infData"}->{"contact:email"};#print "\n$regemail3\n";
         $voiceb=$config4->{response}->{resData}->{"contact:infData"}->{"contact:voice"};#print "\n$voiceb\n";
         $regemail4=$config5->{response}->{resData}->{"contact:infData"}->{"contact:email"};#print "\n$regemail4\n";
         $voicet=$config5->{response}->{resData}->{"contact:infData"}->{"contact:voice"};#print "\n$voicet\n";
...
if($row1[4] eq "dummy\@domains.qa") {$row1[4]="Contact Not Provided";}
...
$E6 \"From: mchinwal\@cra.gov.qa\";
$E6 \"To: mchinwal\@cra.gov.qa\";
$E6 \"Subject: CRA Qatar Domains Contacts Auditing \| Please Verify Your Domain Contacts\";
